...
class TravelAgent:
    ...
    def _parse_hotels(self, hotels_text: str) -> List[Dict[str, str]]:
        ...
        for block in hotel_blocks:
            ...
            hotel = {
                "name": self._extract_field(block, [r'酒店名称[：:]\s*(.+?)(?:\n|$)', r'(?:^|\n)\s*(?:\d+[\.、])?\s*([^\n]+?酒店[^\n]*)', r'(?:^|\n)\s*(?:\d+[\.、])?\s*([^\n]{2,20})(?:\n|$)']),
            ...
            if not hotel["name"]:
                hotel["name"] = block.strip()[:30] + "..." if len(block.strip()) > 30 else block.strip()
        ...
        if not hotels:
            hotels.append({
                ...
                "price": "根据预算选择",
                "features": "多种选择",
                "reason": hotels_text[:200] + "..." if len(hotels_text) > 200 else hotels_text
            })
    ...
    def _parse_transportation(self, transport_text: str) -> Dict[str, str]:
        """解析交通建议文本为结构化数据"""
        return {
            "to_city": self._extract_field(transport_text, [
                r'到达.*?交通[：:]\s*(.+?)(?:\n|$)',
                r'到达.*?方式[：:]\s*(.+?)(?:\n|$)',
                r'(?:飞机|高铁|火车|自驾).*?(?:\n|$)'
            ]) or "详见下方建议",
            "in_city": self._extract_field(transport_text, [
                r'城市内.*?交通[：:]\s*(.+?)(?:\n|$)',
                r'市内.*?交通[：:]\s*(.+?)(?:\n|$)',
                r'(?:地铁|公交|打车|出租车).*?(?:\n|$)'
            ]) or "详见下方建议",
            "between_attractions": self._extract_field(transport_text, [
                r'景点.*?交通[：:]\s*(.+?)(?:\n|$)',
                r'各景点.*?交通[：:]\s*(.+?)(?:\n|$)'
            ]) or "详见下方建议",
            "cost_estimate": self._extract_field(transport_text, [
                r'费用估算[：:]\s*(.+?)(?:\n|$)',
                r'交通费用[：:]\s*(.+?)(?:\n|$)',
                r'(?:约|大约)?\s*\d+\s*元'
            ]) or "根据实际选择"
    ...
    def plan_trip(self, user_type: str, city: str, preferences: List[str], budget: int, days: int) -> Dict[str, Any]:
        # 构建统一的提示词，一次调用获取所有信息
        combined_prompt = f"""
        ...
        hotels = self._parse_hotels(hotels_text) if hotels_text else [{
            "name": "酒店推荐",
            "location": "根据行程区域选择",
        ...
        transportation = self._parse_transportation(transport_text) if transport_text else {
        ...
        return {
            "itinerary": [
                {
                    "day": i+1,
                    "activities": daily_itineraries[i] if i < len(daily_itineraries) else "暂无内容"
                } for i in range(days)
            ],
            "hotels": hotels,
