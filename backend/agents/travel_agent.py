...
class TravelAgent:
    ...
    def plan_trip(
        ...
        **重要提示：请特别考虑时间因素对旅游体验的影响：**
        ...
        ===酒店推荐===
        **重要：请严格遵守用户的预算限制！** 用户总预算为{budget}元，行程{days}天，平均每天预算约为{budget//days}元。
        
        请详细推荐3-5家合适的酒店，每家酒店必须包含以下信息，并且按照指定格式输出：
        酒店名称：[酒店名称]
        位置：[酒店位置，靠近哪些景点，距离市中心的距离]
        价格：[价格区间，具体到每晚的价格范围，必须符合用户预算，每晚价格不超过{budget//days}元]
        特色：[酒店特色，如设施、服务、风格等]
        推荐理由：[详细的推荐理由，为什么适合该用户的旅游类型和预算]
        
        **预算限制要求：**
        - 推荐的酒店每晚价格必须控制在{budget//days}元以内
        - 优先推荐性价比高的酒店
        - 如果预算有限，可以推荐经济型酒店或民宿
        - 确保酒店价格与用户预算相匹配
